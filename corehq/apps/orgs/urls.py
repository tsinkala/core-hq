...
urlpatterns = patterns('corehq.apps.orgs.views',
    ...
    url(r'^(?P<org>[\w\.-]+)/(?P<team_id>[ \w-]+)/add_member/$', 'orgs_add_member', name='orgs_add_member'),
    url(r'^(?P<org>[\w\.-]+)/(?P<member_id>[\w\.-]+)/remove_member/$', 'orgs_remove_member', name='orgs_remove_member'),
